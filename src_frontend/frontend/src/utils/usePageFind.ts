...
function normalizeWithMap(orig: string) {
  ...
  for (const ch of Array.from(orig)) {
    ...
    for (let i = 0; i < n.length; i++)
      map.push(
        normPieces.join('').length - n.length + i >= 0
          ? map.length
            ? map[map.length - 1]! + 0
            : 0
          : 0,
...
export function usePageFind(containerRef: Ref<HTMLElement | null>, opts: Options = {}) {
  ...
  function searchAll(qRaw: string) {
    ...
    for (const node of nodes) {
      ...
      const { norm, map } = normalizeJa
        ? normalizeWithMap(text)
        : { norm: text, map: [...text].map((_, i) => i) };
  ...
  function bind() {
    ...
    if (observeMutations && containerRef.value) {
      ...
      _observer.value.observe(containerRef.value, {
        childList: true,
        characterData: true,
        subtree: true,
      });
  ...
  const countLabel = () =>
    count.value ? `${Math.max(1, current.value + 1)} / ${count.value}` : '0 / 0';
