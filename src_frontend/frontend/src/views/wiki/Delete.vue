...
<template>
  ...
  <button
    class="btn-head-image"
    title="プレビュー画面へ戻ります。&#10;ショートカット: Ctrl + 2"
    v-on:click="previewRedirect(props.id)"
  >
  ...
  <transition>
    <div id="overlay-delete-warn" v-show="isDeleteModal">
      <div id="content-delete-warn">
        ...
        <p>
          <strong
            >ここからの操作はWikiを削除する操作です。<br />操作をやめる場合 「戻る」
            を選択してください。</strong
          >
        </p>
        <div class="btn-zone">
  ...
  <transition>
    <div id="overlay-message" v-show="isMessageModal">
      <div id="content-message">
        ...
        <div class="btn-zone">
          <button v-on:click="messageModalOpenClose('No Message')" class="btn-modal-yes btn-hover">
            閉じる
          </button>
        </div>
      </div>
