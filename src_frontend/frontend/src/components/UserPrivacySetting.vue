...
import { getUserInfoUrl, userPrivacySettingUrl, userTotpSettingUrl, userTotpVerifyUrl, userTotpDisableUrl } from "@/router/urls";
...
const verifyTotp = async (): Promise<void> => {
    if (token.value === "") {
        messageModalOpenClose("QRコードをGoogle Authenticatorなどで読み取り、アプリケーションに表示されている6桁の数字を入力してください。");
        return;
    }
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            <div class="switch-btn-container" title="アカウントのプライバシー設定を切り替えます。">
                                <div class="private-public-toggle">
                                    <div class="switch" v-on:click="isPrivacyChanged()">
                                        <input v-if="isUserPrivate" type="checkbox" id="switch" v-model="isUserPrivate"
                                            chacked>
                                        <input v-else="isUserPrivate" type="checkbox" id="switch"
                            ...
                            <div class="switch-btn-container" title="アカウントの2段階認証設定を切り替えます。">
                                <div class="private-public-toggle">
                                    <div class="switch" v-on:click="changeTotpQRModal()">
                                        <input v-if="isTotpAuth" type="checkbox" id="switch" v-model="isTotpAuth"
                                            chacked>
    ...
    <div id="overlay-gen-qrcode" v-show="isOpenTotpSetupModal">
        <div id="content-gen-qrcode">
            ...
            <div class="post-code">
                ...
                <input class="post-code-input" v-model="token" maxlength="6" required placeholder="XXXXXX">
