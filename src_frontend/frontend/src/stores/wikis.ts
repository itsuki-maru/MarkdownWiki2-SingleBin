...
export const useWikiStore = defineStore('wiki', {
  ...
  actions: {
    async initList(): Promise<void> {
      try {
        const response = await apiClient.get(wikiDataListCountUrl);
        this.wikiList.clear();
        const wikisData = response.data;
        for (let key in wikisData) {
          this.wikiList.set(wikisData[key]['id'], {
            id: wikisData[key]['id'],
    ...
    async queryWiki(query1: string, query2: string): Promise<void> {
      try {
        ...
        let sortedDsc = new Map(
          [...this.wikiList.entries()].sort((a, b) => (a[0] > b[0] ? 1 : -1)).reverse(),
        );
