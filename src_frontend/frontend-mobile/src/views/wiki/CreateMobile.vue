...
const createWiki = async (): Promise<void> => {
  ...
  try {
    ...
    if (createWikiData.value.is_public) {
      messageModalOpenClose("Wiki（パブリック）を作成しました。このWikiは全ユーザーが閲覧可能です。");
    } else {
      messageModalOpenClose("Wiki（プライベート）を作成しました。このWikiは作成したユーザーだけが閲覧可能です。");
    }
...
<template>
  <div class="head-btn-zone">
    <button class="btn-head-img" v-on:click="onOutCheck"><img :src="`${assetsUrl}home_24.png`" class="btn-img"
      alt="home_24.png"></button>
    ...
    <button class="btn-head-img" v-on:click="onLogout"><img :src="`${assetsUrl}exit_24.png`" class="btn-img"
      alt="exit_24.png"></button>
  </div>
  ...
  <div class="main-container">
    ...
    <div class="title-row">
      <input type="text" id="title-input-text" class="title" required v-model="createWikiData.title" placeholder="タイトル">
    </div>
    <textarea class="textarea" ref="formArea" required v-model="createWikiData.body"
      placeholder="## 内容（マークダウン記法で記述）&#13;&#10;&#13;&#10;本文を記述。" id="wiki-detail"></textarea>
