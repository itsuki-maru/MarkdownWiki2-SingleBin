...
export const useEditRequestWikiStore = defineStore('editRequestWiki', {
  ...
  actions: {
    async initList(): Promise<void> {
      try {
        const response = await apiClient.get(getEditWikiRequestsUrl);
        this.editRequestWikiList.clear();
        const wikisData = response.data;
        for (let key in wikisData) {
          this.editRequestWikiList.set(wikisData[key]['id'], {
            id: wikisData[key]['id'],
    ...
    addEditRequestWiki(wiki: EditRequestWiki): void {
      this.editRequestWikiList.set(wiki.id, {
        id: wiki.id,
      ...
      let sortedDsc = new Map(
        [...this.editRequestWikiList.entries()].sort((a, b) => (a[0] > b[0] ? 1 : -1)).reverse(),
      );
